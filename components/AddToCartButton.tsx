...
export default function AddToCartButton({ product, variant = 'default' }: AddToCartButtonProps) {
  ...
  const baseClasses = 'w-full font-semibold rounded-lg transition-all duration-200'
  const sizeClasses = variant === 'large' ? 'py-4 text-lg' : 'py-2 text-sm'

  const buttonClasses = !product.inStock && product.inStock !== undefined
    ...
    : `${baseClasses} ${sizeClasses} bg-yallashop-yellow text-yallashop-navy hover:bg-yallashop-yellow-light`
